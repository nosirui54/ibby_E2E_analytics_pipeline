...
DB_NAME = os.getenv("DB_NAME", "ibby_analytics_db")
...
try:
    ...
    logging.info(f"stg_orders loaded: {len(stg_orders)} records")

    stg_products.to_sql("stg_products", engine, if_exists="append", index=False, schema="staging")
    logging.info(f"stg_products loaded: {len(stg_products)} records")

    stg_users.to_sql("stg_users", engine, if_exists="append", index=False, schema="staging")
    logging.info(f"stg_users loaded: {len(stg_users)} records")
